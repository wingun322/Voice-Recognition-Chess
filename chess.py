            ...
            for square in rank:
                if square == 0:
                    ...
                    if e_count > 0:
                        result += str(e_count)
                    e_count = 0
                    p_name = self.parts[int(square) if square > 0 else int(square)*(-1)]
    ...
    def valid_move(self, cur_pos, next_pos):
        if cur_pos != None and next_pos != None:
            ...
            if part * self.p_move > 0 and part != 0:
                p_name = self.parts[int(part) if part > 0 else int(part)*(-1)] 
